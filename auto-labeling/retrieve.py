...
def embed_wikipedia_worker(rank, model_name, devices, batch_size, world_size):
    ...
    for fname in tqdm(os.listdir(chunks_dir)[rank::world_size]):
        batch_num = int(fname.split(".")[0])
        embed_path = f"retrieval/20230601.en.wiki_embeddings_{model_name.split('/')[-1]}_r{rank}_b{batch_num}.npy"
        chunk_path = f"retrieval/20230601.en.wiki_chunks_{model_name.split('/')[-1]}_r{rank}_b{batch_num}.npy"
        path = os.path.join(chunks_dir, fname)
        with open(path) as f:
            wiki_chunks = json.load(f)
            
        sentence_embeddings = np.zeros((len(wiki_chunks), model.config.hidden_size))
        for i in tqdm(range(0, len(wiki_chunks), batch_size), position=rank):
            ...
            if (i // batch_size) % save_interval == 0:
                ...
                np.save(chunk_path, wiki_chunks)

        np.save(embed_path, sentence_embeddings)
        np.save(chunk_path, wiki_chunks)
    print(f"Process {rank} finished and saved embeddings to {embed_path} and chunks to {chunk_path}")
